create database ExercicioRevisao2_7 
go
use ExercicioRevisao2_7 
Go
...
data date not null
primary key (idCarro,distanciaPercorrida, data)
...
Select Distinct marca, modelo, Sum(distanciaPercorrida) As distanciaT
From carro Inner Join viagem
On carro.id = viagem.idCarro
Inner Join empresa
On empresa.id = carro.idEmpresa
Group By carro.marca, carro.modelo, viagem.distanciaPercorrida
Order By distanciaT


--Consultar Nomes das empresas que não tem carros cadastrados
Select Distinct empresa.nome
From empresa Inner Join carro
On empresa.id = carro.idEmpresa
Where carro.idEmpresa is not null

--Consultar Marca e modelos dos carros que não fizeram viagens
Select distinct marca, modelo
From carro Inner Join viagem 
On carro.id = viagem.idCarro
Where viagem.idCarro is not null
...
Select Distinct Count(carro.id) As qtd_viagens, empresa.nome
From empresa Left Join carro
On empresa.id = carro.idEmpresa
Left Join viagem
On viagem.idCarro = carro.id
Where viagem.idCarro Is not null
Group By viagem.data, empresa.nome
...
Select Distinct nome, marca, modelo, distanciaPercorrida, Case When (distanciaPercorrida) <1000
												 Then Sum(distanciaPercorrida*10)
												 Else Sum(distanciaPercorrida*15)
												 End As Ganho
From empresa Inner Join carro
On empresa.id = carro.idEmpresa
Inner Join viagem
On viagem.idCarro = carro.id
Group By empresa.nome, carro.marca, carro.modelo, viagem.distanciaPercorrida
